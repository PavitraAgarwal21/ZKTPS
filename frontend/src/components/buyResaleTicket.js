...
export default function BuyResaleTicket(props) {
  let old_nullifierhash_hash = toHex(
    "451080864958729097514591881723044877613218272959133178948076815576142953112"
  );
  let old_commitmenthash_hash = toHex(
    "2952130163293882645100394847402318473054464837513818682981193859388898847175"
  );
  ...
  async function getvalue() {
    let ticket_details = await contract.getTicket(
      toHex(old_commitmenthash_hash)
    );
    ...
    try {
      let txn = await contract.buyResaleTicket(
        new_commitment_hash,
        old_nullifierhash_hash,
        old_commitmenthash_hash
      );
      console.log(txn);
  ...
  return (
    <div>
      <div>hello world</div>
      <button onClick={getarr}>getvalue</button>
    </div>
  );
}
